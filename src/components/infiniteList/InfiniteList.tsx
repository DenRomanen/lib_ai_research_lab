...
function useDynamicSizeGrid(props: UseDynamicSizeGridProps) {
  ...
  const measureRowHeight = useCallback(
    ...
    [rowHeightResizeObserver, measureRowHeightInner]
  );
  ...
  const measureColumnWidth = useCallback(
    ...
    [columnWidthResizeObserver, measureColumnWidthInner]
  );
...
const createItems2 = (items: IItem[]) =>
  items.map((item) => ({
    id: Math.random().toString(36).slice(2),
    ovner: item.ovner,
    columns: Object.values(item).map((key) => ({
      id: Math.random().toString(36).slice(2),
      text: key,
    })),
  }));
...
interface I {
  ...
  ItemComponent: ComponentType<{
    ...
    className?: string;
    itemRow?: IItem;
    // virtualColumns?: IDynamicGridColumnProps[];
  }>;
...
const InfiniteList2: FC<I> = ({
  ...
  const [gridItems] = useState(createItems2(items));
  const scrollElementRef = useRef<HTMLDivElement>(null);
  ...
  const {
    ...
    estimateRowHeight: useCallback(
      () => estimateRowHeight,
      [estimateRowHeight]
    ),
    rowsCount: items.length,
    columnsCount: columnsCount,
    columnWidth: useCallback(() => columsWidth, [columsWidth]),
    getColumnKey: useCallback((index) => index, []),
    getScrollElement: useCallback(() => scrollElementRef.current, []),
    getRowKey: useCallback((index) => gridItems[index]!.id, [gridItems]),
  });
  ...
  return (
    ...
    >
      ...
      >
        {virtualRows.map((virtualRow) => {
          const itemRow = gridItems[virtualRow.index]!;
